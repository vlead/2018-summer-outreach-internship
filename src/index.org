...
** Labels and severity
   ...
   3) Type of issues :
    ...
    4) Severity : At any given time a issue should only have one of
       the severity levels as described below. To change the severity
       of an issue, the existing label of the issue should be unchecked
       and the new severity label should be checked.
       ...
       2) S2 : This label indicates that the issues affects major
          functionality or major data. It could have broken links and
          a field view being inconsistent with its specifications
...
** List of issues to be considered invalid
...
*** Time limit for raising issues
    Students would be allowed to log defects from 30th April till 5th
    May , 2018
