...
** Labels and severity
   ...
   3) Type of issues :
      ...
      4) Severity : At any given time a issue should only have one of
         the severity levels as described below. To change the
         severity of an issue, the existing label of the issue should
         be unchecked and the new severity label should be checked.

         1) S1 : This label indicates that the issue affects critical
            functionality or critical data. Example: Prevention of
            user interaction, unfaithful to the semantics of
            interaction and redirection to the error page, logical
            errors in the experiment simulations.

         2) S2 : This label indicates that the issues affects major
            functionality or major data. It could have broken links
            and a field view being inconsistent with its
            specifications Example: In a form if there is a field
            which is editable but it is not allowing the user to edit
            it.
