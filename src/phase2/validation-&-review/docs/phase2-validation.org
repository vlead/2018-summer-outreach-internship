...
* Introduction
  ...
  source community followed for resolving the issues raised by them.
...
* Link for lab code repositories
  ...
  All the issues which we reviewed to be correct and valid are
  migrated in these repos from the your personal GitLab accounts. You
  are free to pick up any issues whether they are raised by you or by
  any other student. We are not assigning any issue to anybody.
* Status of issues
  At any given time an issue should only be in one of the below
  described states. To change the state of a issue, the current status
  label of the issue should be unchecked and the new status label
  should be checked. All the below described labels would not be
  existing in the beginning. They need to be created initially and
  then can be used as and when you keep responding on the same issue.

  - Open : This label indicates that when an issue is
  ...
  - Fixed : This label indicates that the issue has been fixed by the
  ...
  - Resolved : This label indicates that when validators
    validates/verifies the fixes then they can label the issue as
    Resolved. The students as well as the validators can mark the
    issue as Resolved.

  - Reopen : This label indicates that if an issue surfaces during
    testing in the staging and production environments, the VLEAD team
    would label it as 'Reopen'.

  - Closed : This label indicates that Resolved issues would be
    labelled as 'Closed' by the VLEAD team members after they pass the
    deployment tests. This label means that the issue is resolved,
    tested and approved and is ready to go for production deployment.

* Steps to be followed by the interns
  + Step 1 - Choose a lab for which you want to start resolving the
    issues. Go to the lab repository in [[https://github.com/virtual-labs/][GitHub]] and select the
    respective lab repository.
  + Step 2 - Fork the lab repository so that you have the local copy
    of the lab codebase in your machine.
  + Step 3 - Go to the issues section from the repo and pick up the
  ...
  + Step 5 - After you create the pull request, update the same issue
    ...
    request. Label the issue as *fixed*.
  + Step 6 - Once the issue is labelled as *fixed* , provide the issue
  ...
  + Step 7 - Once the issue gets reviewed and okayed by the 2
  ...
  + Step 8 - After you see the issue labelled as *resolved*, if more
    than one student was working on the issue simultaneously, consider
    the issue as fixed and reviewed and hence no more fixes would be
    accepted after this by any other student.
